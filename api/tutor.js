 ...
 *
 * Accepts POST { messages: [{role, text}], assessmentTitle: string }
 * Returns { success: true, reply: string } or { success: false, error: string }
 *
 * Env var: ANTHROPIC_API_KEY (set in Vercel project settings)
 */

const SYSTEM_PROMPT = `You are a Socratic tutor inside Simplifii-OS, an Australian neuroinclusive education platform.
...
export default async function handler(req, res) {
  ...
  const { messages, assessmentTitle } = req.body || {};
  ...
  // Add assessment context to system prompt
  const systemWithContext = assessmentTitle
