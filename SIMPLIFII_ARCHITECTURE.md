...
* **Visuals:** Center-aligned, high-contrast light theme. Zinc-50 background with zinc-900 text. JetBrains Mono typography reserved for the passphrase field and the Zero-Disclosure banner.
...
|---|---|---|---|
| STAGE 01: Sovereign Handshake | `src/frontend/LandingPage.js` | Shipped | Light-theme zinc-50 gateway with JetBrains Mono passphrase field. Wired to `HistoryOfThought.unlockWithPassphrase` (PBKDF2 600k). Siltbrand Pulse 1px emerald-500 perimeter on focus. UDL toggle persists to `SettingsContext.lodLevel`. Pinned Zero-Disclosure footer. Skip path drops into Ghost Mode; the `NOT VERIFIED` badge surfaces downstream. |
| STAGE 02: Ingestion Drive | `src/frontend/UniversalOnboarding.js` + `src/frontend/MasterDashboard.js` + `src/services/BriefService.js` | Shipped | Grounding component redesigned as the Gravity Well: light-theme zinc-50 dropzone with drag-and-drop, Siltbrand Pulse on drag-over, terminal feedback strings ("Initialising Grounding...", "Neural Map Ready"). Filename unit-code detection in `BriefService.detectUnitCode` groups files into distinct Course Pillars via `BriefService.groupFilesByUnitCode`. Each group flows through handleSprintCreation independently, so dropping the BABS1201 set alongside the BABS1202 set creates two pillars not one. `BriefService.detectAcademicTier` classifies each pillar as `lab`, `research`, `practical`, or `general` from filename plus rawText signals; the tier is stored on `extractionData.academicTier` ready for the Stage 03 Pillar Gallery icons. `handleIngestGrounding` and Grounding both share the same group + tier pipeline. |
